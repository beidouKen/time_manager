...
export class SemanticFrameParser {
  ...
  private detectGoal(input: string): SemanticUserGoal {
    ...
    if (
      ...
      // 不匹配"安排一个X任务"——这是任务创建意图
      !/(安排|创建|新建).*(任务|事项)/.test(input)
    ) {
      return "request_advice";
    }

    // V4.2+: 完成任务意图（优先于创建意图，防止"完成这个任务"被误解为创建）
    if (
      /(完成|做完|搞定|结束|已经完成|已完成).*(任务|这个|它|这件事?)/.test(input) ||
    ...
    // 必须有明确的"查看"词或疑问结尾，避免"今天早上安排X任务"被误匹配
    if (
      /(看下|看看|帮我看|看一下|瞧一下).*(安排|计划|日程)/.test(input) ||
      /(今天|明天|本周|这周).*的?(安排|计划|日程).*(是什么|有哪些|有什么|怎么样|如何)/.test(input) ||
      /^(今天|本周).*(安排|计划|日程)\s*$/.test(input)
    ) {
      return "query_today_schedule";
    }

    // "帮我记录一下/记一下" 与 "帮我安排/安排X任务" 语义等价，归入任务创建路径
    // 注意：查询意图（"看下安排" 等）已在 query_today_schedule 优先拦截
